...
def test_invalid_last_report_mode_is_ignored(tmp_path: Path):
    cfg = tmp_path / "c"
    cfg.mkdir()
    (cfg / "config.yaml").write_text(
        ...
        "last_vendor: mikrotik\n"
        "last_report_mode: nope\n",
        encoding="utf-8",
    )
    prefs, warning = load_user_prefs(cfg)
    assert warning is None
    assert prefs.last_report_mode is None
...
def test_remember_eval_does_not_overwrite_report_language(tmp_path, monkeypatch):
    from omf.session import Session
    from omf.tui import _remember_target
