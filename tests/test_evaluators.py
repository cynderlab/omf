...
def test_insecure_http_observed_includes_interfaces():
    evidence = {"services": ev("services", ServiceList(services=(
        Service(
            name="http",
            enabled=True,
    ...
    r = insecure_services_disabled(evidence, {"forbidden": ["http", "telnet"]}, "fortinet")
    assert r.status == "fail"
    assert r.observed["services"][0]["interfaces"] == ["wan1"]
...
def test_evaluate_missing_capability_is_error():
    check = next(c for c in load_catalog("mikrotik") if c.id == "FW-NTP-001")
