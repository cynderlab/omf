...
def test_unrestricted_accept_skips_established_not_interface_scope():
    payload = PolicyList(
        policies=(
            Policy(
                id="*8",
    ...
    assert not p001.get("skip_interface_scoped")
    assert not p003.get("skip_interface_scoped")
    assert [row["id"] for row in no_any_any_accept(evidence, p001, "mikrotik").observed["policies"]] == [
        "*7",
        "open",
    ]
    assert [
        row["id"]
        for row in no_unrestricted_service(evidence, p003, "mikrotik").observed["policies"]
    ] == ["*7", "open"]


def test_operator_filter_any_any_any_hits_even_when_interface_scoped():
    payload = mikrotik_filter(
        [
            ...
            },
            {
                ...
                "out-interface": "pppoe-guifibages",
            },
            {
                "chain": "forward",
                "action": "accept",
                "in-interface": "Lan/Lan",
                "out-interface": "wg-guifi",
            },
            {
                "chain": "forward",
                "action": "accept",
                "in-interface": "wg-vpns",
                "out-interface": "Lan/Lan",
            },
            {
                "chain": "forward",
                "action": "accept",
            ...
            {"chain": "input", "action": "accept", "protocol": "icmp"},
            {
                "chain": "input",
                "action": "accept",
                "dst-port": "53231,53232,53233",
                "protocol": "udp",
            },
            {"chain": "input", "action": "accept", "in-interface": "Lan/Lan"},
            {"chain": "input", "action": "accept", "in-interface": "wg-vpns"},
            {"chain": "input", "action": "accept", "in-interface": "wg-servers"},
    ...
    assert any_any.status == "fail"
    assert [row["id"] for row in any_any.observed["policies"]] == hits
    assert service_any.status == "fail"
    assert [row["id"] for row in service_any.observed["policies"]] == hits
    assert explicit_deny_present(evidence, p002, "mikrotik").status == "pass"
