...
from omf.baseline.evaluators.policy import _policy_hit
...
def utm_on_accept(evidence, params, vendor) -> CheckResult:
    ...
    return CheckResult(
        ...
        severity="high",
        diagnostic=(
            f"accept policies missing {field} {ids!r}"
            if hits
            else f"{field} is set on accept policies"
        ),
        capability_refs=("firewall_filter",),
        observed={"policies": [_policy_hit(p, {field: getattr(p, field)}) for p in hits]},
    )
