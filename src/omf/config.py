...
class UserPrefs:
    ...
    last_report_mode: Literal["eval", "llm"] | None = None
...
def _parse_prefs(data: object) -> UserPrefs:
    ...
    report_mode = data.get("last_report_mode")
    if report_mode not in _ALLOWED_REPORT_MODES:
        report_mode = None
    ...
    return UserPrefs(
        ...
        last_report_mode=report_mode,  # type: ignore[arg-type]
    )
...
def save_user_prefs(config_dir: Path, prefs: UserPrefs) -> None:
    ...
    payload = {
        ...
        "last_report_mode": prefs.last_report_mode,
    }
    path.write_text(
